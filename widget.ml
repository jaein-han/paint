...
let vpair (w1: widget) (w2: widget) : widget = {
  ...
  handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
    if event_within g e (w1.size ()) then w1.handle g e
      ...
      if event_within g2 e (w2.size ()) then w2.handle g2 e
      else ());
  size = (fun () ->
    let (x1, y1) = w1.size () in
    let (x2, y2) = w2.size () in
...
let make_controller (v: 'a) : 'a value_controller =
  ...
  let change_listener_list = {contents = []} in
  {
    add_change_listener = (fun (f: 'a -> unit) ->
      change_listener_list.contents <- f::change_listener_list.contents);
    get_value = (fun () -> value.contents);
    change_value = (fun c ->
      value.contents <- c;
      List.iter (fun f -> f c) change_listener_list.contents)
  }
...
let slider (init: int)  : widget * int value_controller =
    ...
    (border {
      repaint = (fun (g: Gctx.gctx) ->
              Gctx.fill_rect g (0, 0) (value_controller.get_value (), 20));
      handle =
        (fun (g: Gctx.gctx) (e: Gctx.event) ->
          let (x, y) = Gctx.event_pos e g in
          if Gctx.event_type e = Gctx.MouseDown ||
            Gctx.event_type e = Gctx.MouseDrag then
              value_controller.change_value (max 0 x)
              (* ^^ change value to the x location of the mouse *)
          );
      size = (fun () -> 100, 20)
    },
      value_controller)